...
function protactedTemplate($page) {
    //if ((strpos(ROOT_URL, 'docs.loginradius.com') === false)||(strpos(ROOT_URL, 'lrapidocs.azurewebsites.net') === false)) {
    $rurl = parse_url(ROOT_URL);
    $tempHost = isset($rurl['host']) ? $rurl['host'] : '';
    $whiteListDomains = array('www.loginradius.com','docs.loginradius.com','docs.loginradius.org', 'lrapidocs.azurewebsites.net');
    if (!empty($tempHost)) {
